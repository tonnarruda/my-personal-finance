...
func (d *Database) CreateTransaction(tx structs.Transaction) error {
	...
	INSERT INTO transactions (
		id, user_id, description, amount, type, category_id, account_id, due_date, competence_date, is_paid, observation, is_recurring, recurring_type, installments, current_installment, parent_transaction_id, created_at, updated_at, deleted_at
	) VALUES (
...
func (d *Database) GetTransactionByID(id string, userID string) (*structs.Transaction, error) {
	query := `SELECT id, user_id, description, amount, type, category_id, account_id, due_date, competence_date, is_paid, observation, is_recurring, recurring_type, installments, current_installment, parent_transaction_id, created_at, updated_at, deleted_at FROM transactions WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`
...
func (d *Database) GetAllTransactionsByUser(userID string) ([]structs.Transaction, error) {
	query := `SELECT id, user_id, description, amount, type, category_id, account_id, due_date, competence_date, is_paid, observation, is_recurring, recurring_type, installments, current_installment, parent_transaction_id, created_at, updated_at, deleted_at FROM transactions WHERE user_id = $1 AND deleted_at IS NULL ORDER BY due_date DESC, created_at DESC`
...
func (d *Database) UpdateTransaction(id string, userID string, tx structs.Transaction) error {
	query := `UPDATE transactions SET description=$1, amount=$2, type=$3, category_id=$4, account_id=$5, due_date=$6, competence_date=$7, is_paid=$8, observation=$9, is_recurring=$10, recurring_type=$11, installments=$12, current_installment=$13, parent_transaction_id=$14, updated_at=$15, deleted_at=$16 WHERE id=$17 AND user_id=$18`
	_, err := d.db.Exec(query,
		tx.Description,
		tx.Amount,
		tx.Type,
...
func (d *Database) GetInitialTransaction(accountID string, userID string) (*structs.Transaction, error) {
	query := `SELECT id, user_id, description, amount, type, category_id, account_id, due_date, competence_date, is_paid, observation, is_recurring, recurring_type, installments, current_installment, parent_transaction_id, created_at, updated_at, deleted_at FROM transactions WHERE account_id = $1 AND user_id = $2 AND description = 'Saldo Inicial' AND deleted_at IS NULL ORDER BY created_at ASC LIMIT 1`
	var tx structs.Transaction
